...
SELECT cli.id_cliente, cli.nome, cli.idade, prod.produto, prod.valor
FROM tb_clientes AS cli 
INNER JOIN tb_pedidos AS ped ON (cli.id_cliente = ped.id_cliente)
INNER JOIN tb_pedidos_produtos AS pepr ON (ped.id_pedido = pepr.id_pedido)
LEFT JOIN tb_produtos AS prod ON (pepr.id_produto = prod.id_produto);
